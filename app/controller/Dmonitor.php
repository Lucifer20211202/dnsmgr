...
class Dmonitor extends BaseController
{
    public function overview()
    {
        ...
        $switch_count = Db::name('dmlog')
            ->where('date', '>=', date("Y-m-d H:i:s", strtotime("-1 days")))
            ->count();
        $fail_count = Db::name('dmlog')
            ->where('date', '>=', date("Y-m-d H:i:s", strtotime("-1 days")))
            ->where('action', 1)
            ->count();
    ...
    public function taskinfo()
    {
        ...
        if ($task['type'] == 3) {
            ...
            $task['action_name'] = [
                '未知', '<font color="red">切换备用解析记录</font>', '<font color="green">恢复主解析记录</font>'
            ];
        } else {
    ...
    public function mailtest()
    {
        ...
        $result = MsgNotice::send_mail($mail_name, '邮件发送测试。',
            '这是一封测试邮件！<br/><br/>来自：'.request()->root(true));
        if ($result === true) {
    ...
    public function clean()
    {
        ...
        if (request()->isPost()) {
            ...
            Db::execute("DELETE FROM `".config('database.connections.mysql.prefix')."dmlog` WHERE `date`<'".date("Y-m-d H:i:s",
                    strtotime("-".$days." days"))."'");
