    ...
    public function opipform()
    {
        ...
        if (request()->isPost()) {
            if ($action == 'add') {
                ...
                if (Db::name('optimizeip')->where('did', $task['did'])->where('rr', $task['rr'])->where('id', '<>',
                    $id)->find()) {
                    return json(['code' => -1, 'msg' => '当前域名的优选IP任务已存在']);
                }
                ...
                try {
                    ...
                    Db::name('optimizeip')->where('id', $id)->update(['status'     => 1, 'errmsg' => null,
                                                                      'updatetime' => date('Y-m-d H:i:s')
                    ]);
                    return json(['code' => 0, 'msg' => '优选任务执行成功：'.$result]);
                } catch (Exception $e) {
                    Db::name('optimizeip')->where('id', $id)->update(['status'     => 2, 'errmsg' => $e->getMessage(),
                                                                      'updatetime' => date('Y-m-d H:i:s')
                    ]);
                    return json(['code'  => -1, 'msg' => '优选任务执行失败：'.$e->getMessage(),
                                 'stack' => $e->__toString()
                    ]);
                }
            } else {
        ...
        $domains = [];
        foreach (Db::name('domain')->alias('A')->join('account B', 'A.aid = B.id')->field('A.*')->where('B.type', '<>',
            'cloudflare')->select() as $row) {
