...
class Domain extends BaseController
{
    ...
    public function domain_data()
    {
        ...
        $total = $select->count();
        $rows = $select->fieldRaw('A.*,B.type,B.remark aremark')->order('A.id', 'desc')->limit($offset,
            $limit)->select();
    ...
    {
        $id = input('param.id/d');
        $drow = Db::name('domain')->where('id', $id)
            ->find();
        if (!$drow) {
    ...
    public function record_batch_edit()
    {
        ...
        if ($action == 'value') {
            ...
            foreach ($recordinfo as $record) {
                $recordid = $dns->updateDomainRecord($record['recordid'], $record['name'], $type, $value,
                    $record['line'], $record['ttl'], $record['mx'], $record['weight'], $record['remark']);
                if ($recordid) {
                    $this->add_log($drow['name'], '修改解析',
                        $type.'记录 '.$record['name'].' '.$value.' (线路:'.$record['line'].' TTL:'.$record['ttl'].')');
                    $success++;
                } else {
                    $fail++;
                }
            ...
            if ($action == 'line') {
                ...
                $fail = 0;
                foreach ($recordinfo as $record) {
                    $recordid = $dns->updateDomainRecord($record['recordid'], $record['name'], $record['type'],
                        $record['value'], $line, $record['ttl'], $record['mx'], $record['weight'], $record['remark']);
                    if ($recordid) {
                        $this->add_log($drow['name'], '修改解析',
                            $record['type'].'记录 '.$record['name'].' '.$record['value'].' (线路:'.$line.' TTL:'.$record['ttl'].')');
                        $success++;
                    } else {
                        $fail++;
                    }
    ...
    public function record_batch_add()
    {
        ...
        if (request()->isAjax()) {
            ...
            foreach ($recordlist as $record) {
                ...
                if ($recordid) {
                    $this->add_log($drow['name'], '添加解析',
                        $thistype.'记录 '.$arr[0].' '.$arr[1].' (线路:'.$line.' TTL:'.$ttl.')');
                    $success++;
                } else {
                    $fail++;
                }
    ...
    public function record_log()
    {
        ...
        View::assign('domainId', $id);
        View::assign('domainName', $drow['name']);
        return View::fetch('log');
    }

    private function add_log($domain, $action, $data)
    {
        Db::name('log')->insert(['uid'  => request()->user['id'], 'domain' => $domain, 'action' => $action,
                                 'data' => $data, 'addtime' => date("Y-m-d H:i:s")
        ]);
    }
}