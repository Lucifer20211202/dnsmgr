...
class aliyun implements DnsInterface
{
        ...
        if ($data) {
            ...
            foreach ($data['DomainRecords']['Record'] as $row) {
                $list[] = [
                    ...
                    'UpdateTime' => isset($row['UpdateTimestamp']) ? date('Y-m-d H:i:s',
                        $row['UpdateTimestamp'] / 1000) : null,
                ];
            }
            return ['total' => $data['TotalCount'], 'list' => $list];
        }
    ...
    public function getDomainRecordInfo($RecordId)
    {
        ...
        if ($data) {
            return [
                ...
                'UpdateTime' => isset($row['UpdateTimestamp']) ? date('Y-m-d H:i:s',
                    $data['UpdateTimestamp'] / 1000) : null,
            ];
        }
        return false;
    }
    ...
    public function addDomainRecord(
        ...
        $param = [
            ...
            'Type'       => $Type,
            'Value'      => $Value,
            'Line'       => $this->convertLineCode($Line),
            'TTL'        => intval($TTL)
        ];
        if ($MX) {
    ...
    //获取解析记录操作日志
    public function getDomainRecordLog(
        $PageNumber = 1,
        $PageSize = 20,
        $KeyWord = null,
        $StartDate = null,
        ...
        if ($data) {
            ...
            foreach ($data['RecordLogs']['RecordLog'] as $row) {
                $list[] = [
                    'time' => date('Y-m-d H:i:s', intval($row['ActionTimestamp'] / 1000)), 'data' => $row['Message']
                ];
            }
            return ['total' => $data['TotalCount'], 'list' => $list];
        }
    ...
    public function getRecordLine()
    {
        ...
        if ($data) {
            ...
            foreach ($data['RecordLines']['RecordLine'] as $row) {
                $list[$row['LineCode']] = [
                    'name' => $row['LineDisplayName'], 'parent' => $row['FatherCode'] ?? null
                ];
            }
            return $list;
        }
