...
class huawei implements DnsInterface
{
    ...
    public function getDomainRecords(
        ...
        if ($data) {
            ...
            foreach ($data['recordsets'] as $row) {
                ...
                if ($row['type'] == 'MX') {
                    [$row['mx'], $row['records']] = explode(' ', $row['records'][0]);
                }
                $list[] = [
                    'RecordId'   => $row['id'],
    ...
    public function getDomainRecordInfo($RecordId)
    {
        ...
        if ($data) {
            ...
            if ($data['type'] == 'MX') {
                [$data['mx'], $data['records']] = explode(' ', $data['records'][0]);
            }
            return [
                'RecordId'   => $data['id'],
    ...
    private function processLineList(&$list, $line_list, $parent, $rootId = null, $rootName = null)
    {
        foreach ($line_list as $row) {
            ...
            if (!empty($row['children'])) {
                $this->processLineList($list, $row['children'], $row['id'], $rootId === 1 ? $row['id'] : $rootId,
                    $rootName === 1 ? $row['zh'] : $rootName);
            }
        }
    }
    ...
    {
        if (!empty($query)) {
            $query = array_filter($query, fn($a) => $a !== null);
        }
        if (!empty($params)) {
            $params = array_filter($params, fn($a) => $a !== null);
        }
