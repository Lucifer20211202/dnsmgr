...
class OptimizeService
{
    private static $line_name = [
        ...
        'huawei'  => [
            'DEF' => 'default_view', 'CT' => 'Dianxin', 'CU' => 'Liantong', 'CM' => 'Yidong', 'AB' => 'Abroad'
        ],
        'west'    => ['DEF' => '', 'CT' => 'LTEL', 'CU' => 'LCNC', 'CM' => 'LMOB', 'AB' => 'LFOR'],
        'dnsla'   => [
            'DEF' => '', 'CT' => '84613316902921216', 'CU' => '84613316923892736', 'CM' => '84613316953252864',
            'AB'  => ''
        ],
    ...
    public function get_ip_address($cdn_type = 1, $ip_type = 'v4')
    {
        ...
        $response = get_curl($url, json_encode($params), 0, 0, 0, 0, 0,
    ...
    public function execute()
    {
        ...
        foreach ($list as $row) {
            try {
                ...
                Db::name('optimizeip')->where('id', $row['id'])->update(['status'     => 1, 'errmsg' => null,
                                                                         'updatetime' => date('Y-m-d H:i:s')
                ]);
                echo '优选任务'.$row['id'].'执行成功：'.$result."\n";
            } catch (Exception $e) {
                Db::name('optimizeip')->where('id', $row['id'])->update(['status'     => 2,
                                                                         'errmsg'     => $e->getMessage(),
                                                                         'updatetime' => date('Y-m-d H:i:s')
                ]);
    ...
    public function execute_one($row)
    {
        ...
        foreach ($ip_types as $ip_type) {
            ...
            $drow = Db::name('domain')->alias('A')->join('account B', 'A.aid = B.id')->where('A.id',
                $row['did'])->field('A.*,B.type,B.ak,B.sk,B.ext')->find();
            if (!$drow) {
    ...
    private function process_dns_line($dns, $row, $record_list, $get_ips, $line_name, $ip_type)
    {
        ...
        $records = array_filter($record_list, fn($v) => $v['Line'] == $line_name);

        //删除CNAME记录
        $cname_records = array_filter($records, fn($v) => $v['Type'] == 'CNAME');
        ...
        //处理A/AAAA记录
        $ip_records = array_filter($records, fn($v) => $v['Type'] == ($ip_type == 'v6' ? 'AAAA' : 'A'));
        ...
        if (!empty($del_ips)) {
            foreach ($ip_records as $record) {
                if (in_array($record['Value'], $del_ips)) {
                    ...
                    if ($add_ip) {
                        $res = $dns->updateDomainRecord($record['RecordId'], $row['rr'],
                            $ip_type == 'v6' ? 'AAAA' : 'A', $add_ip, $line_name, $row['ttl']);
                        if (!$res) {
        ...
        if ($correct_count < $record_num && !empty($add_ips)) {
            foreach ($add_ips as $add_ip) {
                $res = $dns->addDomainRecord($row['rr'], $ip_type == 'v6' ? 'AAAA' : 'A', $add_ip, $line_name,
                    $row['ttl']);
                if (!$res) {
